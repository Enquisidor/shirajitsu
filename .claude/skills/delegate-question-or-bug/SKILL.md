---
description: Delegate a question or bug fix to the most relevant domain expert agent rather than answering or solving it directly. Invoked by the orchestrator whenever the human asks a question or points out a bug about architecture, product, code, testing, security, infrastructure, or any other domain that belongs to a specific agent.
...
Use this skill whenever the human asks a question or points out a bug during an orchestration session. The orchestrator is a coordinator, not a domain expert — delegate to the agent that holds the relevant expertise.
...
| Question/Bug type | Agent | Persona path |
|---|---|---|
