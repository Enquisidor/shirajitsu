Run the day-zero project setup workflow. Invoke once at project start to produce spec artifacts, assembled personas, and a failing test skeleton. After this completes, hand off to the orchestrator to begin Phase 5 (Implementation).

**Prerequisite:** Run `/setup-agents` before this command. The configurator produces `.agents/config.yml`, `../.claude/agents/assembled/`, and pre-populated `.spec/` stubs that this workflow builds on.
...
After Step 5, the project is ready for the development pipeline:
...
Use `/milestone-kickoff` instead. Do not re-run this command for features added to a project that already has a populated `.spec/` directory and a test suite — that is scope creep into existing project state, not a fresh bootstrap.
